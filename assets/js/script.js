...
function createTaskCard(task) {
    ...
    task.forEach((card) => {
        const today = dayjs();
        const taskDate = dayjs(card.date);
        let dateClass = '';

        // Calculate the difference in days between today and the task's due date
        const daysDifference = taskDate.diff(today, 'day');

        // Check if the task is overdue, nearing the deadline, due today, or due within the next 24 hours
        if (daysDifference < 0) {
            // Task is overdue
            dateClass = 'overdue';
        } else if (daysDifference < 1) {
            // Task is due within the next day (including today)
            dateClass = 'warn';
        } else if (daysDifference === 1) {
            // Task is due tomorrow
            dateClass = 'warn';
        } else if (daysDifference <= 3) {
            // Task is nearing the deadline (within 3 days)
            dateClass = 'nearing-deadline';
        } else if (daysDifference === -1) {
...
// Format the tomorrow's date
const formattedTomorrow = tomorrow.format('YYYY-MM-DD');

// Calculate the difference between two dates
const diffInDays = tomorrow.diff(now, 'day');

console.log('Current date and time:', formattedDate);
console.log('Tomorrow\'s date:', formattedTomorrow);
